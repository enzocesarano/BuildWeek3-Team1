...
import { Button, Modal, Form, Card, Tooltip, Image, Popover, OverlayTrigger, Placeholder } from "react-bootstrap";
import { FaRegImage, FaRegCalendarAlt, FaCertificate, FaUserTie, FaAngleRight } from "react-icons/fa";
...
const CenterHomeTest = ({ loggedInUserId }) => {
  ...
  const arrayAllProfiles2 = useSelector((state) => state.arrayAllProfiles.arrayAllProfiles);
                      ...
                      <Card.Body className="card-container bg-light">
                        <div className="card-home-header mb-3 align-items-center">
                          <div className="d-flex align-items-center">
                            ...
                            >
                              <Image src={element.image} alt="" className="w-100 object-fit-cover" />
                            </div>
                            ...
                            <div>
                              ...
                              <div className="card-home-subtitle text-start">@{post.username}</div>
                            </div>
                          </div>
                          <div className="button-title">
                            <button type="button" className="btn btn-light text-primary card-header-button">
                              Segui
                            </button>
                        ...
                        {post.image && (
                          <div className="mb-4 w-100">
                            <Image src={post.image} className="w-100 rounded-2" alt={post._id} />
                          </div>
                        )}

                        <Card.Footer className="text-muted card-home-footer text-end">
                          <p className="mb-0">Pubblicato il {new Date(post.createdAt).toLocaleString()}</p>
                        </Card.Footer>
                        <div className="card-home-button border border-0 border-bottom border-1 border-secondary-subtle mb-3">
                          ...
                          <button type="button" className="btn  text-dark" onClick={() => handleComment(post._id)}>
                            <ChatLeftText className="m-2" />
                            Commenti {""} {comments.filter((comment) => comment.elementId === post._id).length}
                          </button>
                          <button type="button" className="btn  text-dark">
                        ...
                        {visibleComments === post._id && <CommentArea key={i} post={post} />}
                      </Card.Body>
                    </Card>
                  );
                }
